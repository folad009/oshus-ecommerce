...
import { vendorNavItems, vendorProfile } from "@/data/vendor";
...
export function VendorSidebar({ open, onClose }: VendorSidebarProps) {
  ...
  const navContent = (
    <>
      <div className="px-5 py-6 border-b border-white/10">
        ...
        />
        <p className="text-white/50 text-xs mt-2">Vendor Portal</p>
      </div>

      <div className="px-5 py-3 border-b border-white/10">
        <p className="text-xs text-white/50">Your store</p>
  ...
  return (
    <>
      <aside className="hidden lg:flex flex-col w-[260px] bg-navy-light shrink-0 min-h-screen sticky top-0">
        {navContent}
      </aside>
      ...
      <aside
        className={cn(
          "lg:hidden fixed inset-y-0 left-0 z-50 w-[260px] bg-navy-light flex flex-col transition-transform duration-300",
...
export function VendorMenuButton({ onOpen }: { onOpen: () => void }) {
  return (
    <button
      type="button"
      onClick={onOpen}
