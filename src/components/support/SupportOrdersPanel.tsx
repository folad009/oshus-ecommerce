"use client";

import { useEffect, useState } from "react";
import { OrdersLookupTable } from "@/components/support/OrdersLookupTable";
import type { SupportOrderLookup } from "@/data/support";

export function SupportOrdersPanel() {
  const [orders, setOrders] = useState<SupportOrderLookup[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  useEffect(() => {
    let cancelled = false;

    async function load() {
      try {
        const res = await fetch("/api/support/orders");
        const data = (await res.json()) as {
          orders?: SupportOrderLookup[];
          error?: string;
        };

        if (!res.ok) {
          if (!cancelled) {
            setError(data.error ?? "Failed to load orders.");
          }
          return;
        }

        if (!cancelled) {
          setOrders(
            (data.orders ?? []).map((order) => ({
              ...order,
              status:
                order.status.charAt(0).toUpperCase() + order.status.slice(1),
            }))
          );
        }
      } catch {
        if (!cancelled) {
          setError("Failed to load orders. Is the backend running?");
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
        }
      }
    }

    void load();

    return () => {
      cancelled = true;
    };
  }, []);

  if (loading) {
    return (
      <p className="text-sm text-muted-foreground py-8 text-center">
        Loading orders…
      </p>
    );
  }

  if (error) {
    return (
      <p className="text-sm text-coral py-8 text-center" role="alert">
        {error}
      </p>
    );
  }

  return <OrdersLookupTable orders={orders} />;
}
